...
% save elevation mask_on receiver_table
...
load("benchmark.mat")
plot_2d_traj("scatter", receiver_table, 'b', 'No elevation mask (Benchmark)')
load("skymask_on.mat")
plot_2d_traj("scatter", receiver_table, 'r', 'With elevation mask')
...
plot(Nr_Sat, 'b', 'DisplayName', 'No elevation mask (Benchmark)' , LineWidth=1);
load("skymask_on.mat")
% Nr_Sat = receiver_table.ns;
[~, ~, Nr_Sat] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot(Nr_Sat, 'r', 'DisplayName', 'With elevation mask' , LineWidth=1);
...
plot((1:1:size(gt_ecef, 1)), err_2d, 'b-', 'LineWidth', 1.5, 'DisplayName','No elevation mask (Benchmark)')  % blue solid line with width 2
load("skymask_on.mat"); [err_2d, err_2d_statis(2,:), ~] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot((1:1:size(gt_ecef, 1)), err_2d, 'r-', 'LineWidth', 1.5, 'DisplayName','With elevation mask')  % blue solid line with width 2
set(gcf, 'Position', [100, 100, 500, 500]); % Resizes the current figure
legend('show')
xlabel('Time (epoch)')
