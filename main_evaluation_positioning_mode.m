...
% receiver_table = array2table(receiver_data, 'VariableNames', columnNames);
% 
% save posmode_static receiver_table
...
load("benchmark.mat")
plot_2d_traj("scatter", receiver_table, 'b', 'Kinematic (Benchmark)')
load("posmode_static.mat")
plot_2d_traj("scatter", receiver_table, 'magenta', 'Static')
load("posmode_single.mat")
plot_2d_traj("scatter", receiver_table, 'cyan', 'Single')
load("posmode_diff.mat")
plot_2d_traj("scatter", receiver_table, 'r', 'Differential')

set(gcf, 'Position', [100, 100, 1000, 650]); % Resizes the current figure
...
plot(Nr_Sat, 'b', 'DisplayName', 'Kinematic (Benchmark)' , LineWidth=1);
load("posmode_static.mat")
[~, ~, Nr_Sat] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot(Nr_Sat, 'magenta', 'DisplayName', 'Static' , LineWidth=1);
load("posmode_single.mat")
[~, ~, Nr_Sat] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot(Nr_Sat, 'cyan', 'DisplayName', 'Single' , LineWidth=1);
load("posmode_diff.mat")
[~, ~, Nr_Sat] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot(Nr_Sat, 'r', 'DisplayName', 'Differential' , LineWidth=1);

set(gcf, 'Position', [100, 100, 1000, 300]); % Resizes the current figure
xlabel('Time (epoch)')
ylabel('Number of valid satellite signal')
ylim([0 1.6*max(Nr_Sat)])
legend('show');
grid on;
...
plot((1:1:size(gt_ecef, 1)), err_2d, 'b-', 'LineWidth', 1.5, 'DisplayName','Kinematic (Benchmark)')  % blue solid line with width 2
load("posmode_static.mat");  [err_2d, err_2d_statis(2,:), ~] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot((1:1:size(gt_ecef, 1)), err_2d, 'magenta-', 'LineWidth', 1.5, 'DisplayName','Static')  % blue solid line with width 2
load("posmode_single.mat");  [err_2d, err_2d_statis(3,:), ~] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot((1:1:size(gt_ecef, 1)), err_2d, 'cyan-', 'LineWidth', 1.5, 'DisplayName','Single')  % blue solid line with width 2
load("posmode_diff.mat");  [err_2d, err_2d_statis(4,:), ~] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot((1:1:size(gt_ecef, 1)), err_2d, 'r-', 'LineWidth', 1.5, 'DisplayName','Differential')  % blue solid line with width 2

set(gcf, 'Position', [100, 100, 600, 450]); % Resizes the current figure
legend('show')
xlabel('Time (epoch)')
...
ylabel('2D Error (meter)')
grid on
saveas(gcf, [this_folder,'\err_2d_zm.png']);
