...
% receiver_table = array2table(receiver_data, 'VariableNames', columnNames);
% 
% save consellation_single receiver_table
...
load("benchmark.mat")
plot_2d_traj("scatter", receiver_table, 'b', 'Multiple constellation (Benchmark)')
load("constellation_single.mat")
plot_2d_traj("scatter", receiver_table, 'r', 'Single constellations')
...
plot(Nr_Sat, 'b', 'DisplayName', 'Multiple constellation (Benchmark)' , LineWidth=1);
load("constellation_single.mat")
[~, ~, Nr_Sat] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot(Nr_Sat, 'r', 'DisplayName', 'Single constellation' , LineWidth=1);
...
plot((1:1:size(gt_ecef, 1)), err_2d, 'b-', 'LineWidth', 1.5, 'DisplayName','Multiple constellation (Benchmark)')  % blue solid line with width 2
load("constellation_single.mat");  [err_2d, err_2d_statis(2,:), ~] = get_2d_err(gt_ecef, gt_table, receiver_table);
plot((1:1:size(gt_ecef, 1)), err_2d, 'r-', 'LineWidth', 1.5, 'DisplayName','Single constellation')  % blue solid line with width 2
set(gcf, 'Position', [100, 100, 500, 500]); % Resizes the current figure
legend('show')
xlabel('Time (epoch)')
